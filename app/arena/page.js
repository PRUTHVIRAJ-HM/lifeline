...
export default function ArenaPage() {
  ...
  const handleGenerateAICourse = async () => {
    ...
    try {
      ...
      const numChapters = aiCourseData.chapters === 'auto' ? Math.floor(Math.random() * 8) + 5 : aiCourseData.chapters
...
Make sure the course is tailored to ${aiCourseData.level} level and aligns with the learning goals. Each chapter should build upon the previous one.`
      ...
      try {
        ...
        courseData = {
          ...
          chapters: Array.from({ length: numChapters }, (_, i) => ({
            ...
            topics: ['Topic overview', 'Key concepts', 'Practical examples'],
            duration: '1-2 hours'
          }))
        }
      }
